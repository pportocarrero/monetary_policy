\PassOptionsToPackage{table}{xcolor}
\documentclass[final,a4paper]{report}
\overfullrule=1mm

\usepackage[right=0in, left=0in, top=0in, bottom=0in, headheight=26pt]{geometry}
\usepackage{pgfplots, pgfplotstable}
\usepackage{caption}
\captionsetup{labelsep=space,justification=justified,singlelinecheck=off}
\usepackage{fancyhdr}
\usepackage{csvsimple}
\usepackage{longtable}
\usepackage{booktabs}
\usepackage[outline]{contour} %used for logo, potentially move
\usepackage[detect-all]{siunitx}
\usepackage[eulergreek]{sansmath}
\usepackage{colortbl}
\usepackage{array}
\usepackage{xcolor}
\usepackage{titlesec}
\usepackage{color}
\usepackage{amssymb}
\usepackage{multicol}
\usepackage{microtype}
\usepackage{tgbonum}
\usepackage{tgtermes}
\usepackage{fontawesome}
\usepackage[us]{datetime}
\usepackage{dcolumn}
\usepackage{datenumber}
%\usepackage{fontspec}
\usepackage[colorlinks,
	linkcolor=cyan!50!blue,
	filecolor=cyan!50!blue,
	citecolor=cyan!50!blue,
	urlcolor=lightblue,
	linktoc=graydarker, pdfencoding=auto]{hyperref}

% Color referencing
\definecolor{red1}{RGB}{237,50,55}
\definecolor{lightblue}{RGB}{5,148,185}
\definecolor{graylight}{RGB}{210,211,213}
\definecolor{graydark}{RGB}{96,96,98}
\definecolor{graydarker}{RGB}{75,75,77}
\definecolor{blacklight}{RGB}{55,52,53}
\definecolor{blackdark}{RGB}{47,48,49}

%   TikZ externalize
\usetikzlibrary{pgfplots.external, backgrounds, shadows, pgfplots.dateplot, 
	patterns, pgfplots.fillbetween, pgfplots.groupplots} 
\tikzexternalize[prefix=tikz/, up to date check=diff]
%	\tikzset{external/force remake}

	% Paragraph spacing
\setlength{\parskip}{4pt}
% Sans serif font
\renewcommand*\sfdefault{cmss}
\renewcommand{\familydefault}{\sfdefault}
% Special font for title
\newcommand{\setfont}[2]{{\fontfamily{#1}\selectfont #2}}
% Custom section titles
\titleformat{\section}{\large \bfseries \color{blackdark}}{}{cm}
{\hspace{-0.7cm} \includegraphics{sources/bar.pdf}}
% Custom chart titles
\newcommand{\ct}[2][]{
	\noindent \includegraphics{sources/box.pdf} \color{black!90} \small{
		{#1} \hfill \includegraphics{sources/box.pdf} {#2}}}
% Custom chart footer
\newcommand{\cf}[2][]{
	\indent \color{black!90} \scriptsize{
		{#1} \hfill {#2}}}
% Custom toc title
\titleformat{\chapter}{\large \bfseries \color{red1}}{}{0cm}{}[\vspace{-12mm}]	
% Chapter spacing
\setcounter{secnumdepth}{-1}
% Remove TOC chapter numbers
%\renewcommand{\thesection}{}

% Style for headers and footers, first page large header, no page number
%\makeatletter
%\setlength{\@fptop}{0pt}
%\makeatother

% First page
\fancypagestyle{firststyle}{
%	\pagenumbering{gobble}
	\headheight = 26pt
	\headsep = 12pt
	\fancyhf{}
	\renewcommand{\headrulewidth}{0pt}
	\lhead{ %
		\vspace{1.8cm} \includegraphics[width=21.6cm]{sources/header12vf.png} \medbreak {\hspace{0.3cm} {\large \textbf Monthly Monetary Update \textcolor{red1}{$\vert$ SUMMARY}}}\large \hspace{7.6cm}\today\ $\vert$ \currenttime\ GMT-5}
	\chead{}
	\rhead{}
	\rfoot{}
	\cfoot{}
	\lfoot{}
	}

% Main style			
\fancypagestyle{main}{
	\headheight = 26pt
	\headsep = 12pt
	\fancyhf{}
	\renewcommand{\headrulewidth}{0pt}
	\lhead{ %
		\vspace{1.8cm} \includegraphics[width=21.6cm]{sources/header12vf.png}}
	\chead{}
	\rhead{}
	\rfoot{}
	\cfoot{}
	\lfoot{}
	}

% Last style bc of number issue in footer
\fancypagestyle{last}{
	\headheight = 16pt	
	\fancyhf{}
	\lhead{\Large{\textcolor{cyan!70!blue}
			{\textbf{Macro and Markets Dashboard:}} United States}}
	\rfoot{\footnotesize{\thepage}}
	\cfoot{\raisebox{-\height}}
	\lfoot{	\begin{minipage}[c]{0.05\textwidth}
			\includegraphics[width=\textwidth]{sources/01_bdlogo} 
		\end{minipage}
		\begin{minipage}[c]{0.25\textwidth}
			$\vert$ \footnotesize{\href{http://www.bd-econ.com/}
				{BD Economics}}
		\end{minipage}\vspace{0pt} 
}}

\begin{document}

%% DETAILS
	\begin{minipage}{0.58\textwidth} \vspace{4cm}\setfont{qag}

\setfont{textrm} {\textcolor{red1} {\bfseries {\Large Central Reserve Bank of Peru keeps its benchmark interest rate at 3.00\% as inflation nears the lower end of the target range}}}\\ %% TITLE OF THE REPORT. YOU CAN CHANGE THIS

\bfseries \textcolor{red1}{\LARGE \normalfont Highlights}\\ % HIGHLIGHTS TITLE. YOU CAN CHANGE THE HIGHLIGHTS
\large{
	- Inflation continued its downward trend, with the headline price index at 1.25\% yoy.\\
	- Indicators of business confidence improved slightly in January.
	}\\

\normalfont \bfseries \textcolor{red1}{\LARGE Details}\\ % DETAILS TITLE AND CONTENT. YOU CAN CHANGE THE CONTENT
\small \normalfont{
The Board of the Central Reserve Bank of Peru decided to leave unchanged its benchmark monetary policy rate at 3.00\% (see Exhibit 1) on declining inflation and economic activity below its potential. On January, BCRP cut its rate 25bp as inflation lagged.\\

This decision takes into account that: (i) supply shocks reversed, and as a consequence, inflation dropped further in January; (ii) inflation expectations are also declining; (iii) economic activity is growing below its potential; (iv) global economic activity continued showing favorable indicators; (v) positive signs related to commodity prices; and (vi) capital inflows to emerging economies, although international financial markets are experiencing some uncertainty.\\

In its statement, the Board informed it would pay close attention to new information on inflation and its determinants, to consider if deemed necessary, further changes to the monetary policy stance.\\

Inflation measurements continued showing a downward trend as previous months; the headline price index was 0.13\% for January (prev. 0.16\%) and 1.25\% yoy. In the first month of 2018, all consumption groups saw price gains, with the exception of transport and communication prices (-0.84\% for the month and -0.14\% yoy). House rents, oil and electricity prices advanced the most, with 0.50\% for the month and 3.33\% yoy. Excluding food and energy prices, inflation came negative at -0.13\% (prev. 0.29\%) and 1.97\% yoy (prev. 2.15\%), right in the middle of the target range (see Exhibit 2).\\

The BCRP expects inflation will continue to decline around the lower end of the range, but will converge to 2\% in the medium and long term. Inflation expectations for the next 12 months eased further to 2.23\% (prev. 2.30\%, see Exhibit 3). Inflation expectations for 2018 and 2019, as reported by analysts, kept their previous levels of 2.4\% and 2.5\%, respectively. Business confidence on the economy for the next 3 and 12 months improved slightly in January (see Exhibit 4).\\

You can find more information at (in Spanish):\\
\href{http://www.bcrp.gob.pe/docs/Transparencia/Notas-Informativas/2018/nota-informativa-2018-02-08-1.pdf}{http://www.bcrp.gob.pe/docs/Transparencia/Notas-Informativas/2018/nota-informativa-2018-02-08-1.pdf}
}
	\end{minipage} %\hspace{-2.41mm}
%% END DETAILS

	\begin{minipage}{0.36\textwidth} 
	
		\vspace{4cm}

%% KEY NUMBERS TABLE

		\large \bfseries \textcolor{red1}{Key numbers} \normalcolor \small \normalfont
		\vspace{0.3cm} 
		
		\noindent \footnotesize 
		\color{black!100}
		\rowcolors{1}{}{black!7}
		{\renewcommand{\arraystretch}{1.4}
			\begin{tabular}{ l r r r r }
				& & \normalsize \textsubscript{\begin{tabular}{c}
						change \\ over prev.
				\end{tabular}}& &
				\normalsize \textsubscript{\begin{tabular}{c}
						\vspace{1mm}\\ as of: \end{tabular} } \\ \hline
				&  & \normalsize \textsubscript{\begin{tabular}{c}
						one week\\ change \end{tabular} } & 
				\normalsize \textsubscript{\begin{tabular}{c}
						one month\\ change \end{tabular} }& \normalsize \textsubscript{\begin{tabular}{c}
						\vspace{1mm}\\ as of: \end{tabular} } \\ \hline 

				\hline
			\end{tabular}
		}
		
		\vspace{2mm}
		\color{black!80} Source: Central Reserve Bank of Peru and National Institute of Statistics and Informatics of Peru. \\
		\textsuperscript{1} As reported by economic analysts on the BCRP survey. \\
		\textsuperscript{2} Is the result of the simple average of 12-month expectations by the financial system and economic analysts.\\
		\textsuperscript{3} Index on business confidence surveyed by the BCRP.\\
%% END KEY NUMBERS TABLE

%% TABLE OF CONTENTS
		\tableofcontents
%% END TABLE OF CONTENTS
	\end{minipage} \hspace{7mm}

\thispagestyle{firststyle}
%% CONTACT INFORMATION SECTION
		\vspace{10.35cm}

		\tikzset{external/export next=false}		
		\begin{tikzpicture}[clip=false] \hspace{-0.55cm}
		\node[draw=none, fill=black!10, inner sep=10pt,
	align=justify] (contact) {
			\large \bfseries \textcolor{red1}{Contact Information} \\
				\vspace{0.5mm}
			\quad \color{black!7} . \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!100}{\bfseries Investor Relations Office \hspace{0.5cm}}  \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!100}{\bfseries Directorate of Analysis and Strategy \hspace{0.5cm}}  \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!100}{\bfseries General Directorate of Public Indebtedness and Treasury \hspace{0.5cm}}  \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!90}{Ministry of Economy and Finance \hspace{0.5cm}} \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!90}{+51 1 3115930 ext. 2885 \hspace{0.5cm}} \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!90}{\href{mailto:investor@mef.gob.pe}{investor@mef.gob.pe} \hspace{0.5cm}} \\
				\vspace{0.5mm}
			\quad \normalsize \color{black!90}{\href{https://www.mef.gob.pe/investor.php}{https://www.mef.gob.pe/investor.php} \hspace{0.5cm}}
			 };
		\draw [line width=.15cm, color=red1] (contact.south east) -- (contact.north east);
		\end{tikzpicture}

%% END CONTACT INFORMATION SECTION

	\newpage
	
\thispagestyle{main}
	
	\begin{minipage}{0.50\textwidth} 	\vspace{4mm}


%	\vspace{10cm}
	
		\small
		\section{\textcolor{blackdark}{About the Dashboard}}
		\vspace{-1mm}
		
		\small \normalfont
		The Macro and Markets Dashboard for the United States contains more than 100 indicators of the performance of the U.S. economy. The dashboard includes twelve categories of charts, and a table of exchange rates.  \hfill \vspace{4mm}
		
		Dashboard plots follow a uniform design. The line plots show trends over the past decade, while the horizontal bar plots decompose recent developments. The latest value and its reference period are listed in the bottom left corner of the plot area for line plots. Below each plot is a brief listing of the source with a link (in blue) to the source data. 

	\end{minipage}

\end{document}